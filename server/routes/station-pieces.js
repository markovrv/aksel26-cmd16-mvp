...
// POST /api/station-pieces/:companyId/complete
router.post("/:companyId/complete", auth, [
  body("pieceIndex").isInt({ min: 0, max: 3 }).withMessage("pieceIndex от 0 до 3"),
  validate,
], (req, res, next) => {
  try {
    const db = getDb();
    const { companyId } = req.params;
    const { pieceIndex, choice } = req.body;

    // Find the piece by company_id and piece_index
    const piece = db.prepare(
      "SELECT * FROM station_pieces WHERE company_id = ? AND piece_index = ? AND is_active = 1 LIMIT 1"
    ).get(companyId, pieceIndex);
    if (!piece) return res.status(404).json({ error: "Блок не найден" });

    const selectedOption = choice === "business" ? 0 : choice === "career" ? 1 : 0;
    ...
    // Ensure user_progress row exists BEFORE updating score
    let progress = db.prepare("SELECT * FROM user_progress WHERE user_id = ?").get(req.user.id);
    if (!progress) {
      db.prepare("INSERT INTO user_progress (user_id, score) VALUES (?, 0)").run(req.user.id);
      progress = db.prepare("SELECT * FROM user_progress WHERE user_id = ?").get(req.user.id);
    }
